...
def play_hard(secret, start_time,counter)
  matches= []
  misses= []
  puts "I have generated a sequence with six elements made up of:(p)ink, (r)ed,(g)reen,(b)lue,and (y)ellow. Use (q)uit at any time to end the game. What's your guess? (If you want to cheat, type C)"
  user_guess= gets.chomp
  if (user_guess=="C" || user_guess=="c")
    puts secret.join(",")
    play_hard(secret, start_time,counter)
  elsif (user_guess=="q" || user_guess=="Q")
    puts "That's too bad, I'll re-route you to the main menu"
    master_mind
  elsif user_guess.length < 6
    counter+=1
    puts "That's not long enough, remember, 6 characters"
    play_hard(secret, start_time, counter)
  elsif user_guess.length > 6
    counter+=1
    puts "That's too long. Remember, only 6 characters."
    play_hard(secret,start_time,counter)
  else
    counter+=1
